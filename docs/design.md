...
An observed source-disconnect test left an incomplete compressed frame in the final MP4 even though the file had been finalized. The lab retains the received footage without repairing or re-encoding it. A matching checksum proves byte identity, and an archived state confirms the stored copy; neither establishes that every picture can be decoded. The explicit [recording-health command](recording-health.md) now checks completed local video in bounded batches and saves a separate dated result. Full decoding after an orderly shutdown must be distinguished from this unexpected-input-loss case.
...
7. **Automatic recorded-video health and continuity checks.** The explicit batch decoder is implemented. Before making it automatic, measure its effect on live work and queue growth. Detecting missing scenes or whole missing segments requires additional continuity checks; a successful decode does not establish those facts.
